...
contract ReyaOFTForkCheck is BaseReyaForkTest {
    ...
    function check_ReyaOFT_Permissions(
        ...
        IReyaOFT token = IReyaOFT(sec.reya);
        ...
        // Non-sec.foundationMultisig should not be able to unpause
    ...
    /// @notice Test that sec.foundationMultisig can mint tokens
    ...
    /// @notice Test that non-sec.foundationMultisig cannot mint
    ...
    /// @notice Test that the endpoint has delegated to the sec.foundationMultisig
    function check_ReyaOFT_EndpointDelegatedToOwner() internal view {
        IReyaOFT token = IReyaOFT(sec.reya);
        address endpoint = token.endpoint();
        
        address delegate = ILayerZeroEndpointV2(endpoint).delegates(sec.reya);
        assertEq(delegate, sec.foundationMultisig, "Endpoint should delegate to owner");
        assertEq(token.owner(), sec.foundationMultisig, "Token owner should match");
    }
    ...
    function check_ReyaOFT_SendWorks(
        ...
        // Give sender tokens
        address sender = makeAddr("sender");
        deal(sec.reya, sender, amountToSend);
