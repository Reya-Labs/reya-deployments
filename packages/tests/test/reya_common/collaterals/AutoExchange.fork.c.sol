...
struct TokenBalances {
    ...
    int256 liquidatorBalanceLmToken;
...
contract AutoExchangeForkCheck is BaseReyaForkTest {
    ...
    function check_AutoExchange_LmToken(
        address lmToken,
        bytes32 lmTokenUsdcNodeId,
        ...
        private
    {
        ...
        // deposit rUSD and LM token into user's account
        {
            s.userAccountId = depositNewMA(user, lmToken, 2200e18);
        ...
        // check that the account is AE-able but still healthy
        s.tbal0.userBalanceRusd = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, sec.rusd).marginBalance;
        s.tbal0.userBalanceLmToken = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, lmToken).marginBalance;
        s.tbal0.liquidatorBalanceRusd =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, sec.rusd).marginBalance;
        s.tbal0.liquidatorBalanceLmToken =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, lmToken).marginBalance;
        ...
        assertApproxEqAbsDecimal(
            s.ae1.collateralAmountToLiquidator,
            ud(400e18).div(ud(1e18 - 0.005e18)).div(ud(lmTokenUsdcNodeOutput.price)).unwrap(),
            0.0015e18,
            18
        );

        s.tbal1.userBalanceRusd = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, sec.rusd).marginBalance;
        s.tbal1.userBalanceLmToken = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, lmToken).marginBalance;
        s.tbal1.liquidatorBalanceRusd =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, sec.rusd).marginBalance;
        s.tbal1.liquidatorBalanceLmToken =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, lmToken).marginBalance;

        assertEq(s.tbal1.userBalanceRusd, s.tbal0.userBalanceRusd + 396e6);
        assertEq(s.tbal1.liquidatorBalanceRusd, s.tbal0.liquidatorBalanceRusd - 400e6);
        assertEq(s.tbal1.userBalanceLmToken, s.tbal0.userBalanceLmToken - int256(s.ae1.collateralAmountToLiquidator));
        assertEq(
            s.tbal1.liquidatorBalanceLmToken,
            s.tbal0.liquidatorBalanceLmToken + int256(s.ae1.collateralAmountToLiquidator)
        );
        ...
        assertApproxEqAbsDecimal(
            s.ae2.collateralAmountToLiquidator,
            ud((s.ae2.quoteAmountToAccount + s.ae2.quoteAmountToIF) * 1e12).div(ud(1e18 - 0.005e18)).div(
                ud(lmTokenUsdcNodeOutput.price)
            ).unwrap(),
            0.0015e18,
            18
        );
        ...
        s.tbal2.userBalanceRusd = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, sec.rusd).marginBalance;
        s.tbal2.userBalanceLmToken = ICoreProxy(sec.core).getTokenMarginInfo(s.userAccountId, lmToken).marginBalance;
        s.tbal2.liquidatorBalanceRusd =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, sec.rusd).marginBalance;
        s.tbal2.liquidatorBalanceLmToken =
            ICoreProxy(sec.core).getTokenMarginInfo(s.liquidatorAccountId, lmToken).marginBalance;
        ...
        assertEq(s.tbal2.userBalanceLmToken, s.tbal1.userBalanceLmToken - int256(s.ae2.collateralAmountToLiquidator));
        assertEq(
            s.tbal2.liquidatorBalanceLmToken,
            s.tbal1.liquidatorBalanceLmToken + int256(s.ae2.collateralAmountToLiquidator)
        );
    }

    function check_AutoExchangeRselini_WhenUserHasOnlyRselini() public {
        check_AutoExchange_LmToken(sec.rselini, sec.rseliniUsdcReyaLmNodeId, 0);
    }

    function check_AutoExchangeRselini_WhenUserHasBothRseliniAndRusd() public {
        check_AutoExchange_LmToken(sec.rselini, sec.rseliniUsdcReyaLmNodeId, 100e6);
    }

    function check_AutoExchangeRamber_WhenUserHasOnlyRamber() public {
        check_AutoExchange_LmToken(sec.ramber, sec.ramberUsdcReyaLmNodeId, 0);
    }

    function check_AutoExchangeRamber_WhenUserHasBothRamberAndRusd() public {
        check_AutoExchange_LmToken(sec.ramber, sec.ramberUsdcReyaLmNodeId, 100e6);
    }

    function check_AutoExchangeRhedge_WhenUserHasOnlyRhedge() public {
        check_AutoExchange_LmToken(sec.rhedge, sec.rhedgeUsdcReyaLmNodeId, 0);
    }

    function check_AutoExchangeRhedge_WhenUserHasBothRhedgeAndRusd() public {
        check_AutoExchange_LmToken(sec.rhedge, sec.rhedgeUsdcReyaLmNodeId, 100e6);
    }
