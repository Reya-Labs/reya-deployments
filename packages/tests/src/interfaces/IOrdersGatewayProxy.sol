...
interface IOrdersGatewayProxy {
    ...
    function executeOrders(
        address owner,
        AccountAdvancedOrders[] calldata inputs
    )
        external
        returns (bytes[] memory outputs);

    function grantTradePermission(address target) external;

    function revokeTradePermission(address target) external;
...
struct MatchOrderDetails {
    ...
    uint128 exchangeId;
    /**
     * @dev Account ids of the counterparty(s) to match the order with
     */
    uint128[] counterpartyAccountIds;
    /**
     * @dev Order size in base terms (e.g. in eth units for eth passive perp market)
     */
    int256 baseDelta;
    /**
     * @dev Price limit for the match order, if it's breached then the order will be reverted
     */
    uint256 priceLimit;
}

struct AdvancedOrder {
    MatchOrderDetails matchOrder;
    bool isReduceOnly;
}

struct AccountAdvancedOrders {
    uint128 accountId;
