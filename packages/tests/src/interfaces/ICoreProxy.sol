...
interface ICoreProxy {
    ...
    function setSpotMarketConfiguration(uint128 spotMarketId, SpotMarketConfig memory config) external;

    function getSpotMarketData(uint128 spotMarketId) external view returns (SpotMarketData memory);
    ...
    function getAccountOwnerNonce(uint128 accountId)
        external
        view
        returns (uint256);
    ...
    function getAddressNonce(address owner) external view returns (uint256);
    ...
    function getSelectiveNodeCollateralInfo(
        uint128 accountId,
        address quoteToken,
        address[] memory excludeSupportingCollaterals
    ...
    function setCustomImMultiplierBySystemOwner(
        uint128 accountId,
        /* warning: missing UDVT support in source Solidity version; parameter is `UD60x18`. */
        uint256 imMultiplier
    ) external;
    ...
    error GlobalLockOn();
    error IndexTooLarge(uint256 index, uint256 maxIndex);
    ...
    error AccountTransferNotAllowedDueToNonce(uint128 accountId);
    ...
    error CamelotSwapMinAmountOutBreached(
    ...
    error InvalidCamelotSwapPathLength(uint256 pathLength);
    error UnsafeApproval(address spender, uint256 value);
    error ZeroCamelotSwapAmountIn();
    ...
    function getExchangeInfo(
        ...
        address collateralA,
        address collateralB
    ) external view returns (ExchangeInfo memory);
    ...
    function getSupportingCollaterals(
        uint128 collateralPoolId,
        address quoteToken
    ...
    function isSupportingCollateralOfQuoteCollateral(
        uint128 collateralPoolId,
        address quoteCollateral,
    ...
    function executeBySigViaPublisher(
        uint128 accountId,
        Command[] memory commands,
        EIP712Signature memory sig,
        bytes memory extraSignatureData,
        EIP712Signature memory publisherSig
    )
        external
        returns (bytes[] memory outputs, MarginInfo memory usdNodeMarginInfo);
    ...
    error QuoteOfBlockMismatch(
        uint128 blockId,
        address blockQuoteCollateral,
        address marketQuoteCollateral
    );
    ...
    error UnauthorizedCamelotSwapPublisher(address publisher);
    error UnauthorizedMatchOrderPublisher(address publisher);
    error UnauthorizedTransfer(
        uint128 accountId,
        uint128 destAccountId,
        address accountOwner,
        address destAccountOwner
    );
    ...
    function managePoolStake(uint128 accountId, bytes memory inputs)
        external
        returns (bytes memory);

    error InvalidManagePoolStakeCommand(uint8 command);
    error ZeroStake(uint128 accountId);
    error ZeroUnstake(uint128 accountId);
    ...
    error MulticallFailed(uint256 index, bytes inputData, bytes returnData);
...
enum CommandType {
    ...
    CamelotSwap, // (core command) execute a token swap via camelot
    ManagePoolStake // (core command) stake pool
...
enum Action {
    ...
    StakeTokenized,
    UnstakeTokenized,
    TriggerStakedAssetAutoExchange,
    StakeEnabledAsCollateral,
    StakeDisabledAsCollateral,
    StakeAccountQuote,
    UnstakeAccountQuote
}
...
interface ProtocolConfiguration {
    struct Data {
        ...
        address ordersGatewayAddress;
        address passivePoolAddress;
    }
}
...
struct BackstopLPConfig {
    ...
    uint256 withdrawCooldownDurationInSeconds_DEPRECATED;
    uint256 withdrawDurationInSeconds_DEPRECATED;
}
