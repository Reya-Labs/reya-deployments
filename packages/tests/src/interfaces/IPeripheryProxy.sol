...
import { EIP712Signature } from "./ICoreProxy.sol";
...
interface IPeripheryProxy {
    ...
    function depositLiquidityToAccount(
        DepositLiquidityToAccountInputs memory inputs
    ) external;
    ...
    function lzCompose(
        address oApp, // The OFT token address
        bytes32 guid, // Message GUID
        bytes calldata message, // The composed message
        address executor, // Executor address
        bytes calldata extraData // Extra data
    )
        external
        payable;

    function stakeReya(StakeReyaInputs memory inputs) external;

    function unstakeStakedReya(UnstakeStakedReyaInputs memory inputs) external;
    ...
    function executeViaPublisher(
        PeripheryExecutionViaPublisherInputs memory inputs
    ) external;
    ...
    function withdrawMALZ(WithdrawMALZInputs memory inputs)
        external
        payable
...
enum LzComposerOperationType {
    NONE,
    DEPOSIT_SPOT_ACCOUNT,
    DEPOSIT_ACCOUNT
}

struct StakeReyaInputs {
    uint128 accountId;
    uint256 assetAmount;
    uint256 minShareAmount;
    EIP712Signature withdrawSig;
}

struct UnstakeStakedReyaInputs {
    uint128 accountId;
    uint256 shareAmount;
...
struct DepositLiquidityToAccountInputs {
    uint128 accountId;
    uint128 poolId;
...
struct PeripheryExecutionViaPublisherInputs {
    uint128 accountId;
    Command[] commands;
    EIP712Signature sig;
    EIP712Signature publisherSig;
}
...
struct WithdrawLiquidityFromAccountInputs {
    uint128 accountId;
    uint128 poolId;
    uint256 sharesAmount;
    EIP712Signature sig;
